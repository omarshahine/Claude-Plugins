...
- Get the full email address (e.g., `accounting@seattletennisclub.org`)
...
- "What is the payee name in YNAB?" (e.g., "Seattle Tennis Club")
- "Will this need split transactions (multiple categories) or a single category?"

If Filing Cabinet is selected, ask:
- "What folder name in the Filing Cabinet?" (e.g., "Seattle Tennis Club", "Miles/Phamatech")
...
- "Short name for the action (used in filenames, e.g., 'stc', 'phamatech', 'lf-invoice'):"
- "Display label (shown in triage, e.g., 'Process STC Statement'):"
- "Command name (for the slash command, e.g., 'stc-statement'):"
