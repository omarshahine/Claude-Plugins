...
If groups.route not empty:
  For each route decision:
    ...
    3. If routeInfo.passAttachments:
       → Fetch attachment list via EMAIL_TOOLS.get_email_attachments
    4. Invoke via Task tool with email context
    5. After success: execute routeInfo.postAction
       - "archive" → move to routeInfo.postActionFolder
       - "delete" → delete email
...
- Both set: `Friday (Jane's AI assistant)`
...
| "process", "route" | Process with matched route |
