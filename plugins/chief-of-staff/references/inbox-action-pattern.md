...
|------|--------|--------|------|--------|
| `inbox-action-club-stmt` | accounting@club.example.com | PDF extraction → YNAB split | 3-way split (Dues/Activities/Dining) | Club Statements/ |
| `inbox-action-verify` | verify@testing.example.com | URL extraction → form fill → PDF | No | Verification/ |
| `inbox-action-travel-inv` | notification.invoicing.example.com (+ subject) | PDF extraction → YNAB split | Per-trip split | Travel Agency/ |
| `inbox-action-aviation` | billing@aviation.example.com | Attachment download → rename | No | Aviation/Invoices/ |
