...
4. **PHASE 3 - LEARN (mandatory):** After execution, update data files:
   a. **decision-history.yaml** - Append ALL decisions with: date, emailId, action, senderDomain, senderEmail (if available), folder (if archived), category, accepted (true if user's action matches suggestion, false otherwise), and notes for pattern matches
