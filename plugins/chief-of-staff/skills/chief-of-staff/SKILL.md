...
Action routes extend the triage system to route emails to specialized skills/agents for active processing. While filing rules route emails to folders, action routes route emails to skills.
...
Match found? → Suggest "Process: [label]" as action
    ↓ (user confirms)
Invoke skill/agent via Task tool
    ↓
...
- **Target**: `route.plugin` + `route.agent` or `route.skill` (resolves to Task subagent_type)
...
- **Batch Processor**: Route decisions invoke the target agent/skill via Task tool, then execute the post-action
