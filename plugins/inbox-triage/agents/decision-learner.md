...
description: |
  ...
  - User runs /inbox-triage:learn --from-history
...
**CRITICAL**: First find the plugin data directory by searching for `inbox-triage/*/data/` under `~/.claude/plugins/cache/`. All data files are in that directory:
...
Called via `/inbox-triage:learn --from-history`:
```
