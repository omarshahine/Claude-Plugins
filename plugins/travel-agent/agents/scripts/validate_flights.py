...
def test_total_count_consistency(conn, main_user_id):
    """Verify stats total = tracked (non-superseded) + manual (user-filtered).

    This mirrors the logic in get_flight_stats() in query_flights.py.
    """
    cur = conn.cursor()
    now = datetime.now(tz=timezone.utc).timestamp()
    superseded_ids = get_superseded_flight_ids(conn)

    # Tracked flights (same filter as get_flight_stats)
    cur.execute("""
        SELECT COUNT(*)
        FROM Flight f
...
def test_year_sum_matches_total(conn, main_user_id):
    ...
    cur.execute("""
        SELECT
            ...
            MAX(COALESCE(f.lastKnownDepartureDate, f.departureScheduleGateOriginal))
        FROM Flight f
        JOIN UserFlight uf ON f.id = uf.flightId
        WHERE uf.isMyFlight = 1
          AND uf.deleted IS NULL
